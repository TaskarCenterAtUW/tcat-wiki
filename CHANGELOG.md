...
- **Docs**: Moved and updated docs\assistant\cross-platform\concept\crossing-links.md
...
- **Docs**: Updated Assistant Knowledge Base articles
- **Core**: Optimized avif encoding settings
...
- **Docs**: Refreshed os-connect/data-viewer/user-manual/feedback
...
- **Core**: Completed missing AKB frontmatter keys and normalized required-key ordering across all assistant articles
- **Core**: Fixed the glossary generator's AKB frontmatter key order
- **Core**: Replaced AKB `review_status` with `publication_status` and migrated publication-aware generators, build filtering, tests, and articles
- **Core**: Replaced the ambiguous `draft` authority level with `provisional`
- **Docs**: Ensured every Assistant Knowledge Base `questions` entry was phrased as an article-relevant question
- **Docs**: Updated product-folder index questions to describe their assistant-facing structure and policies rather than product definitions
- **Core**: Repaired malformed YAML frontmatter in the Safe Routes to School Walksheds workflow article
...
- **Docs**: Added new `topics` slugs to the AKB schema's themed vocabulary tables
- **Docs**: Added a `Slug` column to the AKB schema's Product tags table (matched each product's `docs/assistant/{topic}/` directory name), renamed the `topics` section to "Topic tags", and added a "Product slugs" table duplicating those slugs as first-class `topics` values, with a short rationale for the intentional duplication
- **Core**: Added `test_akb_content.py` tests, enforcing that (1) the first `products` entry for single-product topic folders matches that folder's owning product, (2) the first `topics` entry matches the parent topic folder's own slug, and (3) every `products` entry has its schema slug present somewhere in `topics`; `cross-platform/` and `support/` are exempt from checks 1-2 as cross-cutting, multi-product sections
- **Docs**: Expanded the AKB schema's product tags table to list all first-class products, and aligned the generated dispatch registry's `products` frontmatter to match
- **Docs**: Restructured the AKB schema's controlled `topics` vocabulary into themed tables covering every product area
...
- **Core**: Removed explicit Python Markdown association settings file, no longer needed as of Zensical Studio 0.1.2
- **Docs**: Renamed or case-normalized non-conforming `topics` values in AKB articles to match the schema's updated controlled vocabulary
- **Docs**: Reordered AKB article `products`/`topics` frontmatter so the first `products` entry matches the owning product for the article's parent topic folder, the first `topics` entry matches that folder's slug, and every listed product's slug is present in `topics`, per the new `test_akb_content.py` checks; regenerated `assistant/dispatch.md` afterward
- **Docs**: Fixed a stray `QA/QC Reports` `products` value (should be `QA-QC Reports`) and its missing `qa-qc` `topics` slug in `assistant/cross-platform/concept/abbreviations.md`
- **Docs**: Compacted the dispatch registry's status legend table formatting
- **Docs**: Fixed AKB article `products` frontmatter values that did not match the schema's canonical casing
- **Docs**: Updated AKB article `products` frontmatter values for AKB root docs
...
- **Core**: Upgraded Zensical to 0.0.46
...
- **Core**: Updated padding on breadcrumbs and left sidebar sections to prevent clipping of keyboard focus ring
