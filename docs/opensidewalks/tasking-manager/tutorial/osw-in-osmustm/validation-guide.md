...
This section covers how to validate mapped pedestrian infrastructure tasks in the OSM US Tasking Manager for an OpenSidewalks project.

---

### Quick Links

- **Mapping Guide**: [Mapping Guide](mapping-guide.md) (review this before validating!)
- **Join the Team**: [OpenSidewalks Validators Team](https://tasks.openstreetmap.us/teams/26/membership/) (requires approval by TCAT staff)

    ![Screenshot of the OpenSidewalks Validators team page in the OSM US Tasking Manager, showing the team description, organization logo, project cards, and a Join team button at the bottom right.](../../../../resources/images/opensidewalks/tasking-manager/tutorials/osw-in-osmustm/validation-guide/01-join-team-validators-light.png#only-light)
    ![Screenshot of the OpenSidewalks Validators team page in the OSM US Tasking Manager, showing the team description, organization logo, project cards, and a Join team button at the bottom right.](../../../../resources/images/opensidewalks/tasking-manager/tutorials/osw-in-osmustm/validation-guide/01-join-team-validators-dark.png#only-dark)
...
3. **Connected** - Pedestrian features are connected to form a network
...
1. Go to the project page for the project you'd like to contribute to
...
- [ ] Crossings are drawn as **ways** across streets
- [ ] Each crossing has **curb nodes** at both ends
- [ ] A **middle node** connects the crossing to the road
- [ ] Crossing ways are tagged, at minimum: `highway=footway` + `footway=crossing`
...
| Issue                          | How to Fix                                   |
|--------------------------------|----------------------------------------------|
| Crossing not connected to road | Drag the middle node to connect to the road  |
| Missing curb nodes             | Add `barrier=kerb` + `kerb=*` to endpoints   |
| Wrong marking tag              | Check imagery and update `crossing:markings` |

#### 2. Curbs

- [ ] Curbs are **nodes** at the edges of the street
...
| Issue                                  | How to Fix                                                                              |
|----------------------------------------|-----------------------------------------------------------------------------------------|
| Curb directly at sidewalk intersection | Move curbs to actual location and connect sidewalk to curb via a short sidewalk footway |
| Wrong curb type                        | Check street-level imagery if available                                                 |
| Missing `barrier=kerb`                 | Add the tag to complete the feature                                                     |

#### 3. Sidewalks

- [ ] Sidewalks are drawn as **ways** along the center of the path
...
- [ ] Connectors are tagged: `highway=footway` + `footway=sidewalk` + (optionally) `crossing_link=yes`
...
3. If you accidentally move something else or only dragged to check connectivity, select the **Undo** button (keyboard shortcut: ++ctrl+z++)
