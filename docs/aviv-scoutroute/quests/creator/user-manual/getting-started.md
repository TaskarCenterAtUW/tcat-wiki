...
2. **Review** the welcome-page options and the Creator and schema versions shown in the footer

![Quest Definition Creator welcome page showing draft recovery, new-definition, and file or clipboard import controls](../../../../resources/images/aviv-scoutroute/quests/creator/user-manual/getting-started/01-landing-page-light.avif#only-light)
![Quest Definition Creator welcome page showing draft recovery, new-definition, and file or clipboard import controls](../../../../resources/images/aviv-scoutroute/quests/creator/user-manual/getting-started/01-landing-page-dark.avif#only-dark)
...
| :------------------------ | :-------------------------------------------------------------------------------------------- |
| **Resume Current Draft**  | You want to continue a saved draft in this browser; the card appears only when a draft exists |
| **Create New Definition** | You want to start with a blank definition using the Creator's latest-supported schema version |
| **Choose File...**        | You want to load an existing JSON definition from a file                                     |
| **From Clipboard**        | You want to load a valid JSON definition copied to the clipboard                              |
...
Choose how to load the definition:

- **Choose File...** — select a Long Form Quest Definition JSON file from the file picker
- **From Clipboard** — copy valid definition JSON to the clipboard, then select this option; allow clipboard access if the browser requests it

Review the loaded structure in the editor, the schema version in the footer, and the Validation panel. When loading a file, keep the original unchanged until you have reviewed and exported the edited definition.
...
The Creator autosaves edits in the browser's local storage. When a saved draft is available, the welcome page shows a **Resume Draft** card with a summary of its elements and quests. To continue:
