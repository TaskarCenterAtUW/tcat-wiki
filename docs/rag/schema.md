---
title: Assistant corpus schema (RAG)
---

# Assistant corpus schema (RAG)

This document is the authoring contract for [`docs/assistant/`](../assistant/index.md) pages that feed `scripts/export_rag.py` and future indexing pipelines. Product manuals elsewhere in `docs/` keep their existing conventions; only assistant-layer pages must follow this schema end-to-end.
...
Optional subheadings under these sections are allowed if they do not replace the canonical nine headings.
...
`scripts/export_rag.py` emits one JSON object per line (JSONL). Each record includes at minimum:
...
- [Assistant Knowledge Base overview](../assistant/index.md)
