...
2. From the repository root, run:
   ...
   ```powershell
   python utilities/akb-compress-transcript.py <absolute-transcript-path>
   ```
...
2. For each new article, create a complete schema-compliant page:
   ...
   - Use controlled product/topic vocabulary from the schema where applicable. Set `publication_status: stub` when an important issue is unresolved; otherwise use `draft` for newly authored content. Set `last_reviewed` to the current date only when this editing pass is the human editorial pass; otherwise retain or request the appropriate review date.
...
3. For updates, modify only the affected sections and metadata. Keep existing frontmatter unless the transcript-supported change requires an update.
...
- `docs/assistant/dispatch.md` is generated by `utilities/akb-generate-dispatch.py`; it is an inventory, not an authoring guide.
